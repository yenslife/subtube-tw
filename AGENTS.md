...
YouTube URL
→ 下載影片與可用字幕
→ 使用 OpenAI-compatible LLM API 根據上下文翻成台灣繁中 SRT
...
translate_srt.py           # 使用 LLM 翻譯 SRT
...
Model variables depend on `LLM_PROVIDER`: use `OPENAI_MODEL` / `OPENAI_SUMMARY_MODEL` for OpenAI, `OPENROUTER_MODEL` / `OPENROUTER_SUMMARY_MODEL` for OpenRouter.
...
Before upload, `yt_to_zh_video.sh` uses `generate_upload_metadata.py` to create `upload_metadata.json`:

- default title: `(赫米翻譯) {original YouTube title}`
- default description: generated by OpenAI from `metadata.json` + `zh-TW.srt`, in Taiwan Traditional Chinese
- explicit `YOUTUBE_TITLE` / `YOUTUBE_DESCRIPTION` override the generated values unless they are still the old default placeholder values.
